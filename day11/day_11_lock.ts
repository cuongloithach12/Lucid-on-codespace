...
import blueprint from "./bar/plutus.json" with { type: "json" };
...
async function readValidator(): Promise<SpendingValidator> {
  const validator = JSON.parse(await Deno.readTextFile("./bar/plutus.json")).validators[3];
    return {
      type: "PlutusV3",
...
const tx = await lucid
    .newTx()
    .payToContract(scriptAddress, { Inline: param }, { lovelace: 2_300_000n })
    .commit();
  ...
  console.log(`Locked 23_000_000n lovelace to script Tx ID: ${txHash}`);